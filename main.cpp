...
HWND eM; //хендлер для текстового поля Мат. ожтдание
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR args, int ncmdshow)
{
	...
	CreateWindowW(L"myWindowClass", L"Распределение", WS_OVERLAPPEDWINDOW ^ WS_THICKFRAME | WS_VISIBLE, 100, 100, 508, 508, NULL, NULL, NULL, NULL); //создание окна
...
void CreateItemW(HWND hwndList, wchar_t *Text)
{
	...
	ListView_InsertItemW(hwndList, &lvi); //вставка строки в список
}
...
{
	for (int i = 0; i < N; i++) {
		mass[i] = (log(rand() % 1000) / 1000.0) / la;
	}
}
	...
	case WM_COMMAND: { //срабатывает при нажатии на кнопки, и не только
		...
		case BTN_CLICK: { //номер нашей кнопки
			if (!inproces){
				...
				GetWindowTextW(eQuantInt, text, 20); //получаем текст из текстбокса
				int Q = _wtoi(text);
				if (N > 0) {
					double *mass;
					...
					GetWindowTextW(eM, text, 20); //получаем текст из текстбокса
					int M = _wtoi(text);
					mass = new double[N]; //инициализируем массив
					normRaspr(N, mass, G, M); //заполняем его значениями нормального распределения
					qsort(mass, N, sizeof(double), compare); //сортируем полученные значения
					for (int i = 0; i < N; i++) {
						snwprintf(text, 20, L"%f", mass[i]); //преобразуем все значения в строку
					...
					mass = new double[N]; //инициализируем массив
					pokazatRaspr(N, mass, la); //заполняем его значениями нормального распределения
					qsort(mass, N, sizeof(double), compare); //сортируем полученные значения
					for (int i = 0; i < N; i++) {
						snwprintf(text, 20, L"%f", mass[i]); //преобразуем все значения в строку
...
void AddControls(HWND hWnd) {
	...
	CreateWindowW(L"static", L"Нормальное распределение", WS_VISIBLE | WS_CHILD, 17, 116, 199, 17, hWnd, NULL, NULL, NULL);
	...
	CreateWindowW(L"static", L"Мат. ожидание:", WS_VISIBLE | WS_CHILD, 255, 135, 118, 17, hWnd, NULL, NULL, NULL);
	...
	eSigma = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD, 142, 133, 105, 22, hWnd, NULL, NULL, NULL);
	eM = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD, 370, 133, 110, 22, hWnd, NULL, NULL, NULL);
